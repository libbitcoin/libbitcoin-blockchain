    ...
    const transaction_list& transactions)
{
    ...
    while (!validate_block::check_proof_of_work(
...
bool make_signature(transaction_type& tx, size_t input_index,
    const ec_secret& secret, const script_type& script_code)
{
    transaction_input_type& input = tx.inputs[input_index];

    const ec_point public_key = secret_to_public_key(secret);
    if (public_key.empty())
        return false;
    hash_digest tx_hash =
    ...
    if (tx_hash == null_hash)
        return false;
    data_chunk signature = sign(secret, tx_hash, secret);
...
transaction_type create_coinbase(const data_chunk& public_key)
{
    ...
    ++coinbase_nonce;
    null_input.script = raw_data_script(data_chunk{0x00, coinbase_nonce});
...
const std::string private_key =
    "c75cb67b3646e32d0b4fec595daa6b94fe2968316d38ffb91fba415cb9059111";

const data_chunk public_key()
{
    ec_secret secret = decode_hash(private_key);
...
transaction_type construct_transaction(const output_point& prevout)
{
    ...
    for (size_t i = 0; i < tx.inputs.size(); ++i)
    {
        ...
        ec_secret secret = decode_hash(private_key);
        bool sign_success = make_signature(tx, i, secret, prevout_script_code);
    }
    return tx;
}
