...
handle_custom_options()
{
    if [[
        ...
        ($PRESET_ID != "with_consensus") &&
        ($PRESET_ID != "without_consensus") &&
        ($PRESET_ID != "nix-gnu-debug-static-with_consensus") &&
        ($PRESET_ID != "nix-gnu-debug-static-without_consensus") &&
        ($PRESET_ID != "nix-gnu-debug-shared-with_consensus") &&
        ($PRESET_ID != "nix-gnu-debug-shared-without_consensus") &&
        ($PRESET_ID != "nix-gnu-release-static-with_consensus") &&
        ($PRESET_ID != "nix-gnu-release-static-without_consensus") &&
        ($PRESET_ID != "nix-gnu-release-shared-with_consensus") &&
        ($PRESET_ID != "nix-gnu-release-shared-without_consensus") &&
        ($PRESET_ID != "nix-gnu-release-static-size-with_consensus") &&
        ($PRESET_ID != "nix-gnu-release-static-size-without_consensus") &&
        ($PRESET_ID != "nix-gnu-release-shared-size-with_consensus") &&
        ($PRESET_ID != "nix-gnu-release-shared-size-without_consensus")]]; then
        ...
        display_error "  with_consensus"
        display_error "  without_consensus"
        display_error "  nix-gnu-debug-static-with_consensus"
        display_error "  nix-gnu-debug-static-without_consensus"
        display_error "  nix-gnu-debug-shared-with_consensus"
        display_error "  nix-gnu-debug-shared-without_consensus"
        display_error "  nix-gnu-release-static-with_consensus"
        display_error "  nix-gnu-release-static-without_consensus"
        display_error "  nix-gnu-release-shared-with_consensus"
        display_error "  nix-gnu-release-shared-without_consensus"
        display_error "  nix-gnu-release-static-size-with_consensus"
        display_error "  nix-gnu-release-static-size-without_consensus"
        display_error "  nix-gnu-release-shared-size-with_consensus"
        display_error "  nix-gnu-release-shared-size-without_consensus"
        display_error ""
        display_help
        exit 1
    fi
...
{
    display_message "libbitcoin-blockchain installer configuration."
    display_message "--------------------------------------------------------------------"
...
build_all()
{
    ...
    if [[ ! ($CI == true) ]]; then
        ...
        build_from_github_cmake libbitcoin-blockchain ${REPO_PRESET[libbitcoin-blockchain]} "$PARALLEL" true         "yes" "${BITCOIN_BLOCKCHAIN_OPTIONS[@]}" $CUMULATIVE_FILTERED_ARGS_CMAKE "$@"
    else
