[![Build Status](https://github.com/libbitcoin/libbitcoin-blockchain/actions/workflows/ci.yml/badge.svg?branch=version3)](https://github.com/libbitcoin/libbitcoin-blockchain/actions/workflows/ci.yml)

[![Coverage Status](https://coveralls.io/repos/libbitcoin/libbitcoin-blockchain/badge.svg?branch=version3)](https://coveralls.io/r/libbitcoin/libbitcoin-blockchain?branch=version3)

# Libbitcoin Blockchain

*The Bitcoin Blockchain Library*
...
On Linux and macOS libbitcoin-blockchain can be build using Autotools and cmake. This process is accomplished via the provided installation scripts `install.sh` or `install-cmake.sh`. These scripts provide a basis for installation of the dependencies of the project. Both support the use of the argument `--help` for further parameterization.

Installation of libbitcoin-blockchain comes with requirements listed below.
...
A minimal libbitcoin-blockchain build requires boost 1.73.0 to 1.76.0 and libsecp256k1. The [libbitcoin/secp256k1](https://github.com/libbitcoin/secp256k1/tree/version7) repository is forked from [bitcoin-core/secp256k1](https://github.com/bitcoin-core/secp256k1) in order to control for changes and to incorporate the necessary Visual Studio build. The original repository can be used directly but recent changes to the public interface may cause build breaks. The `--enable-module-recovery` switch is required. Both of these are able to be provided via the installation script(s).
...
Below is a breakdown of preparation required to install libbitcoin-blockchain. Once an appropriate [compiler](http://en.wikipedia.org/wiki/Compiler) and [build system](http://wikipedia.org/wiki/GNU_build_system) are in place, proceed to [Install Script Execution](#installscript) below to perform installation.
...
Next install the [Boost](http://www.boost.org) (1.73.0 - 1.76.0) development package. This can be accomplished via parameterization of the installation script during libbitcoin-blockchain installation and it is recommended to the reader that this approach be used.
...
The [autotools install script](https://github.com/libbitcoin/libbitcoin-blockchain/blob/version3/install.sh) and [cmake install script](https://github.com/libbitcoin/libbitcoin-blockchain/blob/version3/install-cmake.sh) are written so that the manual build steps for each dependency can be inferred by a developer.

You can run either install script from any directory on your system. By default this will build libbitcoin-blockchain in a subdirectory named `build-libbitcoin-blockchain` and install it to `/usr/local/`. The install script requires `sudo` only if you do not have access to the installation location, which you can change using the `--prefix` option on the installer command line.

The build script clones, builds and installs five unpackaged repositories, namely:
...
- [libbitcoin/libbitcoin-database](https://github.com/libbitcoin/libbitcoin-database)
- [libbitcoin/libbitcoin-consensus](https://github.com/libbitcoin/libbitcoin-consensus)
- [libbitcoin/libbitcoin-blockchain](https://github.com/libbitcoin/libbitcoin-blockchain)
